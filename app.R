...
seasons <- dbGetQuery(conn, "
  SELECT Season, Height, PlayerID, TeamID, Games, BR, BFRank, ChampionPosition, ChampionID, AA, SeasonNo, covidtopup, nonafl
  FROM [dbo].[Season] 
  WHERE Season >= 2010
  AND (covidtopup IS NULL OR covidtopup != 1)
  AND (nonafl IS NULL OR nonafl != 1)
")
...
draftData <- dbGetQuery(conn, "
    SELECT PlayerID, DraftType, Pick, AFLClub, Year
    FROM AFLDraftHistory
    WHERE PlayerID != '3350'
  ")
...
getExperienceSummary <- function(conn, seasons, players, teamLookup, seasonYear) {
  ...
  gamesPost <- seasons %>%
    ...
    mutate(experienceGroup = case_when(
      ...
      totalGamesAtTime < 100 ~ "50-99",
      totalGamesAtTime < 150 ~ "100-149",
      totalGamesAtTime < 200 ~ "150-199",
      totalGamesAtTime >= 200 ~ "200+"
    ))
}
  ...
  summary %>%
    mutate(
      draftGroup = case_when(
        ...
        DraftType == "National" & Pick >= 41 & Pick <= 50 ~ "41-50",
        DraftType == "National" & Pick > 50 ~ "51+",
        DraftType %in% c("Rookie", "Rookie Elev") ~ "Rookie",
        DraftType %in% c("Mid-Season", "Pre-Season") ~ "Pre/Mid",
        TRUE ~ "Other"
      )
    ) %>%
    group_by(TeamName, draftGroup) %>%
  ...
  summary <- seasons %>%
    ...
    mutate(
      acquiredGroup = case_when(
        ...
        DraftType %in% c("RestFA", "UnRestFA") ~ "RestFA & UnrestFA",
        DraftType == "Rookie" ~ "Rookie",
...
ui <- fluidPage(
  ...
  fluidRow(
    ...
    column(2,
           selectInput("seasonYear", "Select Season", choices = sort(unique(seasons$Season), decreasing = TRUE))
    ),
    column(2,
           selectInput("otherTeamHightlight", "Highlight Team", choices = c(
             ...
             sort(
               ...
               unique(teamLookup$TeamName[teamLookup$TeamID <= 18])
               ,
               decreasing = FALSE
             )
           ))
...
server <- function(input, output, session) {
  ...
  output$experienceSummaryTable <- render_gt({
    ...
    desired_order <- c("0-24", "25-49", "50-99", "100-149", "150-199", "200+")
  ...
  output$heightSummaryTable <- render_gt({
    ...
    df <- heightSummary %>%
      mutate(heightGroup = factor(heightGroup, levels = c("<180", "180-184", "185-189", "190-194", "195-199", "200+"))) %>%
